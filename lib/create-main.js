        ...
        return function(input_data, options)
        {
          ...
          function initializeModules(factories_modules, options)
          {
            ...
            function callSetters(modules)
            {
              ...
              modules.recordSet.setLogger(modules.logger.createInstance('record-set'));
              modules.recordStore.setLogger(modules.logger.createInstance('record-store'));

              modules.resultFormatter
                .setLevel(options.resultLevel)
                .setLogger(modules.logger.createInstance('result-formatter'));
              
              Object.keys(modules.hooks).forEach(function(name) {
                modules.hooks[name]
                  .setLogger(modules.logger.createInstance('hooks/' + name))
            ...
            return Object.seal(Object.freeze(callSetters(utils.callFactories(factories_modules, options.modules))));
          ...
          return pool_workers.proxy().then(function(worker) {
            ...
            function processRecordSet(total_results)
            {
              ...
              {
                return worker.processRecord(record, target_step, options);
              }
                    ...
                    records.forEach(function(record, index) {
                      ...
                      function handleResult(result)
                      {
                        if (records.length === results.push({
                          ...
                          resolveCallback(results.sort(function(a, b) {
              ...
              function processRecord(record)
              {
                ...
                modules_run.logger.info('Start record processing');
              ...
              function processRelatedRecords(records)
              {
                ...
                modules_run.logger.info('Processing related records');
                ...
                return modules_run.hooks.relatedRecordsRetrieved.run(records).then(fn_related_records_pipeline).then(function(results) {
                  ...
                  function iterate(records, results)
                  {
                    ...
                    return record ? record.failed && record.recordStore && options.rollback ? doRollback(record).then(function(result) {
