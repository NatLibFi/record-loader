...
function factory(Promise, Object, jjv, jjve, schema, hookFactory, resultFormatterFactory, createLoggerFactory)
{
	...
	function processRecord(record_data, skip_setting_index_as_processed)
	{
		...
		function setResult(value, step)
		{
		    ...
		    if (config.processing.resultsLevel !== 'total' && value[index_additional] !== undefined) {
			result[step] = modules.resultFormatter.additional(value[index_additional]);
		    }

		    if (config.processing.resultsLevel === 'debug') {
			result.data = modules.resultFormatter.data(step === 'filter' ? record_data.data : value[0]);
		    }
