...
function factory(Promise, Object, shim_array, jjv, jjve, schema, workerpool, utils, hookFactory, resultFormatterFactory, createLoggerFactory)
{
	...
	function startRecordDataTransaction(records, retries)
	{
	    ...
	    /*	    function handleRecordProcessing(record, steps, results)
		    {
		    
		    modules.logger.info('Starting processing of record index ' + record.index);

		    return processRecord(record, PROCESSING_STEPS.indexOf(steps[0]), PROCESSING_STEPS.indexOf(steps[steps.length - 1]), getRecordResult(results, record)).then(function(result) {
		    results.push(result);
		    });
		...
		return Promise.all(records_list.map(function(record) {		    
		    ...
		    return pool_workers.exec('handleRecordProcessing', [record, config, steps, results_processing]);
		    
		})).then(
		    function(results_parallel) {

			results_parallel = results_parallel.reduce(function(result, element) {
			    return result.concat(element.shift());
			}, []);
		...
		function iterate(list, results_processing)
		{
			...
			return pool_workers.exec('handleRecordProcessing', [record, config, steps, results_processing]).then(
			    function(result) {
