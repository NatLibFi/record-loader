...
(function (root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        define(['es6-polyfills/lib/promise', 'loglevel', 'jjv', 'jjve', '../resources/config-schema.json'], factory);
    } else if (typeof module === 'object' && module.exports) {
        module.exports = factory(require('es6-polyfills/lib/promise'), require('loglevel'), require('jjv'), require('jjve'), require('../resources/config-schema.json'));
    }
...
function factory(Promise, log, jjv, jjve, schema)
{
	...
	{	    
		...
		if (excp instanceof Object) {
		    throw new Error("Configuration doesn't validate against schema: "+JSON.stringify(excp, undefined, 2));
		} else {
		    throw excp;
		}
	    }
		...
		if (typeof previous_results_or_first_run === 'object') {
			...
			Object.keys(previous_result).forEach(function(key) {
				...
				Object.keys(previous_result.recordStore).forEach(function(record_store_key) {
				    results.recordStore[record_store_key] += previous_result.recordStore[record_store_key];
				});
	...
	function getRecordData(current)
	{
	    log.debug('getRecordData');
	    if (current) {
		return modules.recordSet.current();
	    } else {
		return modules.recordSet.next().then(function(record_data) {
		    if (record_data.data !== undefined && processed_records_indices.indexOf(record_data.index) >= 0) {
	...
	function handleRecordData(record_data)
	{
	    ...
	    } else if (record_data.data === undefined) {
	...
	function startRecordDataTransaction(records)
	{
		...
		if (record === undefined) {
		    ...
		    return processRecord(record).then(
			function(result) {
	...
	function processRecord(record_data, skip_setting_index_as_processed)
	{
		...
		if (step === undefined) {
		    ...
		    return promise.then(function(load_report) {

			if (!result.skipped) {
			    result.processed = 1;
			}

			if (load_report !== undefined && config.processing.target === 'load') {
			    Object.keys(load_report).forEach(function(key) {
				result.recordStore[key] = load_report[key];
			    });
			}
	...
	try {
	    initialise();
	    return modules.recordSet.initialise(input_data).then(function(){
